...
host: "Experimental Design and Statistics in R, AFEC, XTBG"
institute: "XTBG, CAS"
strip-comments: true
format:
  revealjs:
    ...
    width: 1440
    height: 810
    # min-scale: 1.0   # never shrink below 100%
    max-scale: 1.0   # never grow above 100%
    # margin: 0       # no extra padding around it
...
```{r, fig.height=7}
x <- seq(0, 20)
y <- dpois(x, lambda = 5.4)
tibble(x, y) |>
  ggplot() +
  xlab("k") +
  ylab("Density") +
  ggtitle(expression(P(lambda == 5.4))) +
...
- e.g., when your model is $k_i \sim \mathcal{P}(\lambda = 5.4)$ (Poisson distribution with mean and variance = 5.4), what is the probability mass at k = 8?
:::
...
tibble(x, y) |>
  ...
  xlab("mu") +
  ylab("ln L") +
  ggtitle(expression(Ln~L(mu)~~"for"~~x %~% N(mu, 1))) +
  geom_line(aes(x, y)) +
  theme(
    axis.title   = element_text(size = 32),
    axis.text    = element_text(size = 28))
...
::: footer
Example inspired by
[Nguyen & Katabuchi 2025 *Journal of Forestry Research*](https://doi.org/10.1007/s11676-025-01898-9)
...
- $\textcolor{#7570b3}{P(\mathrm{Parameter})}$
  ...
  - Probability to get our parameters. This is what we assume for our parameter before we see any data (i.e., **[prior??]{.purple}** — next section).
...
$\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\mathcal{N}(\mu \mid 0, 2.5) \times \text{Half-Cauchy}(0, 1)$
...
# Combine into a 3D array: iterations × chains × parameters
...
$p(\theta_1, \theta_2 \mid \text{data}) \propto p(\text{data} \mid \theta_1, \theta_2) \times p(\theta_1, \theta_2)$
...
- Slides & code: <https://github.com/mattocci27/bayes-workshop-xtbg>

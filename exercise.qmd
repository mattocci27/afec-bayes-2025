...
Leaf mass per area (LMA) is an important trait that reflects plant strategies.
LMA for interspecific data often shows a log-normal distribution.
...
These commands compile the models, translating Stan code to C++ and then into machine-executable files.
...
where $\boldsymbol{\beta}$ is a 1 $\times$ 3 coefficient matrix (or row vector) and $\boldsymbol{x_i}$ is a 3 $\times$ 1 predictor matrix (including the intercept).
We use $\boldsymbol{x}$ (3 $\times$ N) to compute the $\mu$ vector with length $N$.

**Q:**
What are the posterior means and 95% credible intervals of the coefficients ($\beta_0$, $\beta_1$, $\beta_2$)?
...
```{r vint-hypo, message=FALSE, echo=FALSE, fig.height=4}
set.seed(12345)
...
Column names for log-transformed species-level intercepts ($\log \beta_{0j}$).
...
```{r vslope-hypo, message=FALSE, echo=FALSE, fig.height=4}
set.seed(12345)
...
grid_vals <- mvtnorm::dmvnorm(
  expand.grid(
    log_beta0 = seq(min(mvn_draws$log_beta0), max(mvn_draws$log_beta0), length.out = 80),
    beta1 = seq(min(mvn_draws$beta1), max(mvn_draws$beta1), length.out = 80)
  ),
  mean = mu_demo,
  sigma = Sigma_demo
)
contour_df <- expand.grid(
  ...
  dplyr::mutate(density = grid_vals)

scatter_plot <- ggplot(mvn_draws, aes(x = log_beta0, y = beta1)) +
  geom_point(alpha = 0.4, colour = "#1f78b4") +
  geom_contour(data = contour_df, aes(z = density), colour = "grey40", alpha = 0.7) +
  labs(
...
hist_log_beta0 <- ggplot(mvn_draws, aes(x = log_beta0)) +
  geom_histogram(fill = "#1f78b4", colour = "white", bins = 30, alpha = 0.75) +
  ...
  theme(
    ...
    plot.margin = margin(b = 2, l = 10, r = 10)
  )

hist_beta1 <- ggplot(mvn_draws, aes(x = beta1)) +
  geom_histogram(fill = "#1f78b4", colour = "white", bins = 30, alpha = 0.75) +
  coord_flip() +
  theme_minimal(base_size = 11) +
  theme(
    ...
    panel.grid.minor = element_blank(),
    plot.margin = margin(t = 10, l = 2, b = 10)
  )

top_row <- (hist_log_beta0 + plot_spacer()) +
  plot_layout(widths = c(5, 1))   # widths for row 1

bottom_row <- (scatter_plot | hist_beta1) +
  plot_layout(widths = c(5, 1))   # widths for row 2
...
**Q:**
What are the species-level scaling factors ($\beta_{0j}$) and scaling exponent ($\beta_{0j}$)?
What are the global scaling exponent ($\mu_0$) and scaling exponent ($\mu_1$)?
...
The inverse-whishart distribution $\mathcal{W}^{-1}$ is a conjugate for the covariance matrix $\boldsymbol{\Sigma}$ in MVN.
However, despite conjugacy, inverse–Wishart priors are often computationally difficult to sample.
...
$$
\boldsymbol{z}_j \sim \mathcal{N}(\mathbf0,\,I_2)
$$
$$
\begin{pmatrix}\log\beta_{0j}\\[3pt]\beta_{1j}\end{pmatrix}
  = \boldsymbol{\mu} + L\, \boldsymbol{z}_j
$$
...
To visualise the varying slopes and intercepts, start by peeking at the posterior draws of the species-level coefficients and then plot the implied DBH–height curves.
...
```{r vslope-cmdstan-curves, fig.height=5.5}
pred_cmd_summary <- dplyr::bind_cols(
  pred_grid,
  purrr::map2_dfr(
    match(pred_grid$sp, species_levels),
...
```{r vslope-brm-coefs, fig.height=4.5, message=FALSE, eval=FALSE}
brm_coef_array <- coef(slope_fit_brm)$sp
find_stat_col <- function(stat_names, candidates) {
  match <- intersect(candidates, stat_names)
  if (length(match) == 0) NA_character_ else match[1]
}
brm_summary_wide <- brm_coef_array |>
  as.data.frame.table(responseName = "value") |>
  dplyr::rename(sp = Var1, coefficient = Var2, stat = Var3) |>
  tidyr::pivot_wider(names_from = stat, values_from = value)

stat_names <- names(brm_summary_wide)
estimate_col <- find_stat_col(stat_names, c("Estimate", "Est.", "Mean", "Average"))
lower_col <- find_stat_col(stat_names, c("Q2.5", "l-95% CI", "2.5%", "Lower", "Lower 95%"))
upper_col <- find_stat_col(stat_names, c("Q97.5", "u-95% CI", "97.5%", "Upper", "Upper 95%"))

brm_summary <- brm_summary_wide |>
  dplyr::mutate(
    estimate = if (!is.na(.env$estimate_col)) .data[[.env$estimate_col]] else NA_real_,
    lower = if (!is.na(.env$lower_col)) .data[[.env$lower_col]] else NA_real_,
    upper = if (!is.na(.env$upper_col)) .data[[.env$upper_col]] else NA_real_
  ) |>
  dplyr::transmute(
    sp = factor(sp, levels = species_levels),
    coefficient = dplyr::recode(coefficient, "Intercept" = "intercept", "log(dbh)" = "slope"),
    estimate,
    lower,
    upper
  )

ggplot(brm_summary, aes(x = estimate, y = sp)) +
  geom_point() +
  ...
  labs(
    ...
    y = NULL,
    title = "Species-specific coefficients from brms"
  )
```

```{r vslope-brm-curves, fig.height=5.5, message=FALSE, eval=FALSE}
...
**H:**
There is a power-law relationship ($y =\beta_0 x^{\beta_1}$) between tree diameter (DBH) and tree maximum height, and both the scaling factor $\beta_0$ and the exponent $\beta_1$ vary among species.
Those species-level variations can be explained by wood density (group-level predictor: $u$) of each species.

```{r vgrp-hypo, message=FALSE, echo=FALSE, fig.height=4}
set.seed(12345)
...
**Q:**
What are the coefficients for species-level regressions ($\gamma_{0}^{(\beta_0)}$, $\gamma_{1}^{(\beta_0}$, $\gamma_{0}^{(\beta_1)}$, and $\gamma_{1}^{(\beta_1}$)?
...
Now we have two group-level predictors (intercept and slope), and $u$ includes the intercept and wood density.
The rest of this section follows the setup from the varying slopes and intercepts section.
```{r vgrp-data}

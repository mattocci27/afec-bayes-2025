...
# CmdStan predicted DBH-height curves per species
...
pred_cmd_summary <- purrr::map_dfr(seq_along(species_levels), function(j) {
  sp_name <- species_levels[j]
  grid_sp <- pred_grid |>
    dplyr::filter(sp == sp_name)
  log_dbh_vals <- log(grid_sp$dbh)
  log_beta0_draw <- cmd_draws[, log_beta0_cols[j]]
  log_pred <- outer(beta1_draws, log_dbh_vals)
  log_pred <- sweep(log_pred, 1, log_beta0_draw, "+")
  pred_height <- exp(log_pred)
  tibble::tibble(
    sp = sp_name,
    dbh = grid_sp$dbh,
    estimate = apply(pred_height, 2, stats::median),
    lower = apply(pred_height, 2, function(x) stats::quantile(x, 0.025)),
    upper = apply(pred_height, 2, function(x) stats::quantile(x, 0.975))
  )
})
